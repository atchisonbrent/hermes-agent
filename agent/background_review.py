...
_SKILL_REVIEW_PROMPT = (
    "Review the conversation for validated reusable learning. A no-write pass "
    "is successful when nothing durable is missing. Identify the existing owner "
    ...
    "  • User corrected style, tone, format or verbosity (for example, 'stop doing X'). "
    "Save a durable preference once in user memory when that tool is available; "
    "otherwise skip persisting that preference. Do not write it to another owner. Only a reusable procedural "
    "correction belongs in the governing skill.\n"
    ...
    "write only when the recurrence and ownership gates pass:\n"
    ...
    "Ownership: stable user preferences belong in user memory; procedures "
    "belong in skills. Do not duplicate preferences across both. Task state "
    "and temporary choices stay in session history.\n\n"
    ...
    "If no validated durable change is needed, say 'Nothing to save.' and stop."
)
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "**Skills**: validated reusable procedures, only for recurring triggers or "
    ...
    "in user memory; validated reusable procedures belong in skills.\n"
    ...
    "and stop. Do not manufacture a learning to justify a write."
)
